...
##3a.read in subjects and activities for test data 
...
##3b.add new column for treatment (test or train)
testcols$treatment <- "test"

##3c.read in subjects and activities for train data, add column for treatment
...
##3d.add all subject, activity & treatment columns together and then add to dataframe
datacols <- rbind(testcols, traincols)
ordered <- cbind(datacols, ordered)

##3e.convert activity factor variable to description of activity
library(car)
...
bySubjectActivity <- group_by(ordered, treatment, subject, activity)
tidysummary <- summarise_each(bySubjectActivity, funs(mean))
